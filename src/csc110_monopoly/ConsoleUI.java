...
public class ConsoleUI {
	...
	static int promptForMenuSelection(String[] options, boolean withQuit) throws IOException 
	{
		...
		int choice = promptForInt("Please enter your choice", min, arrayLen);
	...
	static byte promptForByte(String prompt, byte min, byte max) throws IOException
	{
		...
		while(!valid)
		{
			...
			try
			{
				returnValue = Byte.parseByte(input);
			}
			...
			{
				System.out.println("Please enter a byte within range.");
			}
		}
		return returnValue;
	}
	...
	static short promptForShort(String prompt, short min, short max) throws IOException
	{
		...
		while(!valid)
		{
			...
			try
			{
				returnValue = Short.parseShort(input);
			}
			catch(NumberFormatException ex)
			{
			...
			{
				System.out.println("Please enter a short within range.");
			}
		}
		return returnValue;
	...
	static long promptForLong(String prompt, long min, long max) throws IOException
	{
		...
		while(!valid)
		{
			...
			try
			{
				returnValue = Long.parseLong(input);
			}
			catch(NumberFormatException ex)
			{
			...
			{
				System.out.println("Please enter a Long within range.");
			}
		}
		return returnValue;
	...
	static float promptForFloat(String prompt, float min, float max) throws IOException
	{
		...
		while(!valid)
		{
			...
			{
				System.out.println("Please enter a float within range.");
			}
		}
		return returnValue;
	...
	static double promptForDouble(String prompt, double min, double max) throws IOException
	{
		...
		while(!valid)
		{
			...
			{
				System.out.println("Please enter a double within range.");
			}
		}
		return returnValue;
	...
	static String promptForInput(String prompt, boolean allowEmpty) throws IOException
	{
		...
		while(!valid)
		{
			...
			if((!allowEmpty && returnValue.length() != 0) || allowEmpty)
			{
				valid = true;
			}
		}
	...
	static char promptForChar(String prompt, char min, char max) throws IOException
	{
		...
		while(!valid)
		{
			...
			{
				System.out.println("Please enter a valid character in range.");
				continue;
			}
			
			if(input.length() == 1 && ( (int)returnValue >= (int)min && (int)returnValue <= (int)max) )
			{
				valid = true;
			}
			else
